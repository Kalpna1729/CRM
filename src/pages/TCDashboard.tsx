...
export default function TCDashboard() {
  ...
  const scheduleMeeting = async (reqId: string, leadId: string, boId: string) => {
    ...
    const meetingId = `m${Date.now()}`; // Single ID used everywhere — avoids mismatch
    await addMeeting({
  ...
  const renderBODetail = (boId: string) => {
    ...
    const mobileOff = boLeads.filter(l => l.leadStatus === 'Mobile Off').length;
    const incomingBarred = boLeads.filter(l => l.leadStatus === 'Incoming Barred').length;
  ...
  const renderDetailModal = () => {
    ...
    const { type, title, data } = detailModal;

    const renderContent = () => {
      if (type === 'meetings-list') {
        const items: any[] = data.meetings || [];
        return (
          <div>
            <div style={{ marginBottom: '16px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              {['Converted', 'Scheduled', 'Pending', 'Not Done', 'Follow-Up', 'Meeting Done'].map(s => {
                const count = items.filter((m: any) => m.status === s).length;
                if (!count) return null;
                return <span key={s} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '5px 12px', borderRadius: '20px', background: 'var(--bg3)', border: '1px solid var(--border2)', fontSize: '11px', fontFamily: "'JetBrains Mono', monospace", color: 'var(--text2)' }}>
                  {statusBadge(s)} <span style={{ fontWeight: 700, color: 'var(--text)' }}>{count}</span>
                </span>;
              })}
            </div>
            <div style={{ background: 'var(--bg3)', borderRadius: '12px', border: '1px solid var(--border)', overflow: 'hidden' }}>
              <table className="data-table">
                <thead><tr><th>Client</th><th>Date</th><th>BDM</th><th>BO</th><th>Product</th><th>Type</th><th>Status</th><th>Amount</th></tr></thead>
                <tbody>
                  {items.map((m: any) => {
                    const lead = leads.find(l => l.id === m.leadId);
                    ...
                    return (
                      <tr key={m.id}>
                        ...
                        <td style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '11px', color: 'var(--accent)' }}>{m.date}</td>
                        <td style={{ color: 'var(--success)', fontSize: '12px' }}>{bdm?.name || '—'}</td>
                        <td style={{ color: 'var(--text2)', fontSize: '12px' }}>{bo?.name || '—'}</td>
                        <td><span className="product-chip">{m.productType || '—'}</span></td>
                        <td style={{ fontSize: '11px', color: 'var(--text2)' }}>{m.meetingType || '—'}</td>
                        <td>{statusBadge(m.status)}</td>
                        <td style={{ color: '#3d7fff', fontWeight: 700 }}>₹{lead?.loanRequirement || '—'}</td>
                      </tr>
                    );
                  })}
                  {items.length === 0 && <tr><td colSpan={8} className="empty-row">No meetings found</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
      ...
      if (type === 'meeting-detail') {
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {/* Top identity row */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '16px', background: 'var(--bg3)', borderRadius: '12px', border: '1px solid var(--border)' }}>
              <div style={{ width: '52px', height: '52px', borderRadius: '14px', background: 'rgba(61,127,255,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', fontWeight: 700, color: 'var(--accent)', flexShrink: 0 }}>
                {(m.clientName || lead?.clientName || '?')[0]}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text)', marginBottom: '4px' }}>{m.clientName || lead?.clientName || '—'}</div>
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  <span style={{ fontSize: '11px', color: 'var(--text3)', fontFamily: "'JetBrains Mono', monospace" }}>📞 {lead?.phoneNumber || '—'}</span>
                  <span style={{ fontSize: '11px', color: 'var(--accent)', fontFamily: "'JetBrains Mono', monospace" }}>📅 {m.date} · {m.timeSlot}</span>
                  {m.location && <span style={{ fontSize: '11px', color: 'var(--text3)', fontFamily: "'JetBrains Mono', monospace" }}>📍 {m.location}{m.state ? `, ${m.state}` : ''}</span>}
                </div>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px' }}>
              {[
                { label: 'BDM', val: bdm?.name || '—', color: 'var(--success)', icon: '👤' },
                { label: 'BUSINESS OFFICER', val: bo?.name || '—', color: 'var(--text)', icon: '👤' },
                { label: 'MEETING TYPE', val: m.meetingType || '—', color: 'var(--purple)', icon: '🤝' },
                { label: 'PRODUCT TYPE', val: m.productType || '—', color: 'var(--purple)', icon: '📦' },
                { label: 'LOAN AMOUNT', val: `₹${lead?.loanRequirement || '—'}`, color: '#3d7fff', icon: '💰' },
                { label: 'FINAL REQUIREMENT', val: m.finalRequirement ? `₹${m.finalRequirement}` : '—', color: '#3d7fff', icon: '💰' },
                { label: 'COLLATERAL VALUE', val: m.collateralValue ? `₹${m.collateralValue}` : '—', color: 'var(--warning)', icon: '🏦' },
                { label: 'STATE', val: m.state || '—', color: 'var(--text2)', icon: '🗺️' },
                { label: 'LEAD STATUS', val: lead?.leadStatus || '—', isStatus: true, icon: '📊' },
              ].map(item => (
                <div key={item.label} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: '10px', padding: '14px 14px 12px' }}>
                  <div style={{ fontSize: '9px', color: 'var(--text3)', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '1.5px', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '5px' }}>
                    <span>{item.icon}</span> {item.label}
                  </div>
                  {(item as any).isStatus
                    ? statusBadge(item.val)
                    : <div style={{ fontSize: '14px', fontWeight: 700, color: (item as any).color }}>{item.val}</div>
                  }
                </div>
              ))}
            </div>
      ...
      if (type === 'date-detail') {
        ...
        return (
          <div>
            ...
            <div style={{ background: 'var(--bg3)', borderRadius: '12px', border: '1px solid var(--border)', overflow: 'hidden' }}>
              <table className="data-table">
                <thead><tr><th>Time</th><th>Client</th><th>BDM</th><th>BO</th><th>Product</th><th>Type</th><th>Status</th><th>Amount</th></tr></thead>
                <tbody>
                  {items.map((m: any) => {
                    const lead = leads.find(l => l.id === m.leadId);
                    const bdm = users.find(u => u.id === m.bdmId);
                  ...
                  {items.length === 0 && <tr><td colSpan={8} className="empty-row">No meetings on this date</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
      ...
      if (type === 'request-detail') {
        const { req, lead, bo } = data;
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {/* Identity */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '16px', background: 'var(--bg3)', borderRadius: '12px', border: '1px solid var(--border)' }}>
              <div style={{ width: '52px', height: '52px', borderRadius: '14px', background: 'rgba(0,212,170,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', fontWeight: 700, color: 'var(--success)', flexShrink: 0 }}>
                {(lead?.clientName || '?')[0]}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text)', marginBottom: '4px' }}>{lead?.clientName || '—'}</div>
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  <span style={{ fontSize: '11px', color: 'var(--text3)', fontFamily: "'JetBrains Mono', monospace" }}>📞 {lead?.phoneNumber || '—'}</span>
                  <span style={{ fontSize: '11px', color: 'var(--accent)', fontFamily: "'JetBrains Mono', monospace" }}>💰 ₹{lead?.loanRequirement || '—'}</span>
                  <span style={{ fontSize: '11px', color: 'var(--success)', fontFamily: "'JetBrains Mono', monospace" }}>👤 BO: {bo?.name || '—'}</span>
                </div>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', alignItems: 'flex-end' }}>
                {statusBadge(req.status)}
                {lead?.leadStatus && statusBadge(lead.leadStatus)}
              </div>
            </div>
            {/* Info grid */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px' }}>
              {[
                { label: 'LEAD TYPE', val: lead?.leadType || '—', color: 'var(--purple)', icon: '🏷️' },
                { label: 'NUMBER STATUS', val: lead?.numberStatus || '—', isStatus: true, icon: '📱' },
                { label: 'PRIORITY', val: lead?.priority || 'None', color: lead?.priority === 'Hot' ? '#ff4757' : lead?.priority === 'Warm' ? '#f59e0b' : 'var(--text3)', icon: '🔥' },
                { label: 'CALL COUNT', val: String(lead?.callCount || 0), color: (lead?.callCount || 0) === 0 ? 'var(--danger)' : 'var(--accent)', icon: '📞' },
                { label: 'FOLLOW-UP DATE', val: lead?.followUpDate || 'Not set', color: 'var(--warning)', icon: '📅' },
                { label: 'ADDRESS', val: lead?.address || '—', color: 'var(--text3)', icon: '📍' },
              ].map(item => (
                <div key={item.label} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: '10px', padding: '14px 14px 12px' }}>
                  <div style={{ fontSize: '9px', color: 'var(--text3)', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '1.5px', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '5px' }}>
                    <span>{item.icon}</span> {item.label}
      ...
      if (type === 'leads-list') {
        ...
        const hot = items.filter((l: any) => l.priority === 'Hot').length;
        const warm = items.filter((l: any) => l.priority === 'Warm').length;
        const notCalled = items.filter((l: any) => !l.callCount || l.callCount === 0).length;
        return (
          <div>
            {/* Summary chips */}
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '16px' }}>
              {[
                { label: `${items.length} Total`, color: 'var(--accent)', bg: 'rgba(61,127,255,0.1)' },
                { label: `${items.filter((l: any) => l.leadStatus === 'Interested').length} Interested`, color: 'var(--success)', bg: 'rgba(0,212,170,0.1)' },
                { label: `${hot} Hot`, color: '#ff4757', bg: 'rgba(255,71,87,0.1)' },
                { label: `${warm} Warm`, color: '#f59e0b', bg: 'rgba(245,158,11,0.1)' },
                { label: `${notCalled} Not Called`, color: 'var(--danger)', bg: 'rgba(255,71,87,0.08)' },
              ].map(c => (
                <span key={c.label} style={{ padding: '5px 14px', borderRadius: '20px', background: c.bg, color: c.color, fontSize: '11px', fontWeight: 700, fontFamily: "'JetBrains Mono', monospace" }}>{c.label}</span>
              ))}
            </div>
            <div style={{ background: 'var(--bg3)', borderRadius: '12px', border: '1px solid var(--border)', overflow: 'hidden' }}>
              <table className="data-table">
                <thead><tr><th>Client</th><th>Phone</th><th>BO</th><th>Num. Status</th><th>Lead Status</th><th>Priority</th><th>Calls</th><th>Amount</th></tr></thead>
                <tbody>
                  {items.map((l: any) => {
                    ...
                    return (
                      <tr key={l.id}>
                        <td className="primary">{l.clientName}</td>
                        <td style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '11px', color: 'var(--text3)' }}>{l.phoneNumber}</td>
                        <td style={{ color: 'var(--accent)', fontSize: '12px' }}>{bo?.name || '—'}</td>
                        <td>{statusBadge(l.numberStatus)}</td>
                        <td>{statusBadge(l.leadStatus)}</td>
                        <td>
                          {l.priority
                            ? <span style={{ fontSize: '10px', fontWeight: 700, padding: '2px 8px', borderRadius: '5px', fontFamily: "'JetBrains Mono', monospace", background: l.priority === 'Hot' ? 'rgba(255,71,87,0.12)' : l.priority === 'Warm' ? 'rgba(245,158,11,0.12)' : 'rgba(61,127,255,0.12)', color: l.priority === 'Hot' ? '#ff4757' : l.priority === 'Warm' ? '#f59e0b' : 'var(--accent)' }}>{l.priority}</span>
                            : <span style={{ color: 'var(--text3)', fontSize: '11px' }}>—</span>}
                        </td>
                        <td style={{ fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, color: (l.callCount || 0) === 0 ? 'var(--danger)' : 'var(--accent)' }}>{l.callCount || 0}</td>
                        <td style={{ color: '#3d7fff', fontWeight: 700 }}>₹{l.loanRequirement}</td>
                      </tr>
                    );
                  })}
                  {items.length === 0 && <tr><td colSpan={8} className="empty-row">No leads found</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
              ...
              <div>
                <div style={{ fontSize: '16px', fontWeight: 700, color: 'var(--text)', letterSpacing: '-0.3px' }}>{title}</div>
                {data.subtitle && (
                  <div style={{ fontSize: '11px', color: 'var(--text3)', fontFamily: "'JetBrains Mono', monospace", marginTop: '3px' }}>
                    {data.subtitle}
                  </div>
                )}
              </div>
            ...
            <button onClick={() => setDetailModal(null)} style={{
              ...
              color: 'var(--text2)', cursor: 'pointer', fontSize: '20px',
          ...
          <main className="cc-main">
            ...
            {activeTab === 'overview' && (
              <div className="fade-in">
                  ...
                  {[
                    {
                      ...
                      onClick: () => setDetailModal({ type: 'meetings-list', title: 'All Meetings', data: { meetings: teamMeetings, subtitle: `${teamMeetings.length} meetings in selected range` } })
                    },
                    {
                      ...
                      onClick: () => setDetailModal({ type: 'meetings-list', title: 'Converted Meetings', data: { meetings: teamMeetings.filter(m => m.status === 'Converted'), subtitle: `${convertedCount} conversions` } })
                    },
                    {
                      label: 'Pending', value: pendingCount, cls: 'orange', sub: 'needs follow-up', bars: [3, 4, 3, 5, 3, 4, 4, pendingCount], barColor: '#f59e0b',
                      onClick: () => setDetailModal({ type: 'meetings-list', title: 'Pending / Scheduled Meetings', data: { meetings: teamMeetings.filter(m => m.status === 'Pending' || m.status === 'Scheduled'), subtitle: `${pendingCount} meetings need attention` } })
                    },
                    {
                      ...
                      onClick: () => setDetailModal({ type: 'leads-list', title: 'All Team Leads', data: { leads: allTeamLeads, subtitle: `${allTeamLeads.length} leads across ${myBOs.length} BOs` } })
                    },
                  ].map(k => {
                    const max = Math.max(...k.bars) || 1;
                    return (
                      <div key={k.label} className={`cc-kpi ${k.cls}`} onClick={k.onClick} style={{ cursor: 'pointer' }}>
                        ...
                        <div style={{ fontSize: '9px', color: 'var(--text3)', fontFamily: "'JetBrains Mono', monospace", marginTop: '8px', letterSpacing: '1px' }}>click to view details →</div>
                      </div>
                    );
                  })}
                </div>
                      ...
                      {todayMeetings.filter(m => m.tcId === currentUser?.id).map((m, i, arr) => {
                        ...
                        return (
                          <div key={m.id} className="tl-item" style={{ cursor: 'pointer' }}
                            ...
                            <div className="tl-content" style={{ transition: 'background 0.15s' }}
                ...
                {/* {stateMap.length > 0 && (
                  <div className="glass-card">
                    ...
                    <table className="data-table">
                      ...
                      <tbody>
                        {stateMap.map(row => (
                          <tr key={row.state} style={{ cursor: 'pointer' }}
                            onClick={() => setDetailModal({ type: 'meetings-list', title: `Meetings — ${row.state}`, data: { meetings: teamMeetings.filter(m => (m.state || 'Unknown') === row.state), subtitle: `${row.total} meetings · top product: ${row.topProduct}` } })}>
                ...
                {/* ════ TC PERFORMANCE COMPARISON GRAPH ════ */}
                {(() => {
                  const allTCs = users.filter(u => u.role === 'TC' && u.active);
                  const todayDate = new Date().toISOString().split('T')[0];
                  const COLORS = { scheduled: '#3d7fff', converted: '#00d4aa', pending: '#f59e0b', rescheduled: '#ff6b35', rejected: '#ff4757' };

                  const tcStats = allTCs.map(tc => {
                    const all = meetings.filter(m => m.tcId === tc.id);
                    const tod = all.filter(m => m.date === todayDate);
                    return {
                      id: tc.id, name: tc.name,
                      scheduled: tod.filter(m => m.status === 'Scheduled').length,
                      rescheduled: tod.filter(m => m.status === 'Reschedule Requested').length,
                      pending: tod.filter(m => m.status === 'Pending').length,
                      rejected: tod.filter(m => m.status === 'Reject' || m.status === 'Not Done').length,
                      converted: tod.filter(m => m.status === 'Converted' || m.status === 'Meeting Done').length,
                      todayTotal: tod.length,
                      allTotal: all.length,
                      allConverted: all.filter(m => m.status === 'Converted').length,
                      allPending: all.filter(m => m.status === 'Pending' || m.status === 'Scheduled').length,
                      allRejected: all.filter(m => m.status === 'Reject' || m.status === 'Not Done').length,
                    };
                  });

                  if (allTCs.length === 0) return null;
                  const BAR_H = 30, GAP = 12, LABEL_W = 140, CHART_W = 500;
                  const maxVal = Math.max(...tcStats.map(t => t.todayTotal), 1);
                  const svgH = tcStats.length * (BAR_H + GAP) + 30;

                  return (
                    <div className="glass-card" style={{ marginBottom: '20px' }}>
                      <div className="card-header">
                        <div>
                          <div className="card-title">TC Performance Comparison</div>
                          <div className="card-sub">// all team coordinators · today's breakdown + overall stats</div>
                        </div>
                        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                          {Object.entries(COLORS).map(([k, c]) => (
                            <span key={k} style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', fontSize: '10px', color: 'var(--text2)', fontFamily: "'JetBrains Mono', monospace" }}>
                      ...
                      <div className="card-body">
                        {/* ── Today stacked bar chart ── */}
                        <div style={{ fontSize: '10px', color: 'var(--text3)', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '1.5px', marginBottom: '14px' }}>TODAY'S MEETINGS — STACKED BY STATUS</div>
                        <div style={{ overflowX: 'auto' }}>
                          <svg width={LABEL_W + CHART_W + 60} height={svgH} style={{ display: 'block', minWidth: '500px' }}>
                            {tcStats.map((tc, i) => {
                              const y = i * (BAR_H + GAP) + 10;
                              const segs = [
                                { key: 'scheduled', val: tc.scheduled },
                                { key: 'converted', val: tc.converted },
                                { key: 'pending', val: tc.pending },
                                { key: 'rescheduled', val: tc.rescheduled },
                                { key: 'rejected', val: tc.rejected },
                              ] as { key: keyof typeof COLORS; val: number }[];
                              const isMe = tc.id === currentUser?.id;
                              let xOff = LABEL_W;
                              return (
                                <g key={tc.id}>
                                  {/* Name */}
                                  <text x={LABEL_W - 12} y={y + BAR_H / 2 + 4} textAnchor="end"
                                    fill={isMe ? '#3d7fff' : 'var(--text2)'}
                                    fontSize="11" fontFamily="Inter, sans-serif"
                                    fontWeight={isMe ? '700' : '500'}>
                                    {tc.name.length > 16 ? tc.name.slice(0, 15) + '…' : tc.name}{isMe ? ' ★' : ''}
                                  </text>
                                  {/* Track */}
                                  <rect x={LABEL_W} y={y} width={CHART_W} height={BAR_H} rx="7" fill="var(--bg3)" />
                                  {/* Segments */}
                                  {segs.map((seg, si) => {
                                    if (!seg.val) return null;
                                    const w = (seg.val / maxVal) * CHART_W;
                                    const isFirst = xOff === LABEL_W;
                                    const isLast = si === segs.filter(s => s.val > 0).length - 1;
                                    const r = <rect key={seg.key} x={xOff} y={y} width={w} height={BAR_H}
                                      rx={isFirst ? '7' : isLast ? '7' : '0'}
                                      fill={COLORS[seg.key]} opacity="0.9" />;
                                    xOff += w;
                                    return r;
                                  })}
                                  {/* Count label */}
                                  <text x={xOff + 8} y={y + BAR_H / 2 + 4}
                                    fill="var(--text2)" fontSize="11"
                        ...
                        {/* ── Overall stats table ── */}
                        <div style={{ fontSize: '10px', color: 'var(--text3)', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '1.5px', margin: '24px 0 12px' }}>OVERALL PERFORMANCE — CLICK ROW TO SEE MEETINGS</div>
                        <div style={{ background: 'var(--bg3)', borderRadius: '12px', border: '1px solid var(--border)', overflow: 'hidden' }}>
                          <table className="data-table">
                            <thead>
                              <tr>
                                <th>TC Name</th>
                                <th style={{ color: '#3d7fff' }}>Total</th>
                                <th style={{ color: COLORS.converted }}>Converted</th>
                                <th style={{ color: COLORS.pending }}>Pending</th>
                                <th style={{ color: COLORS.rejected }}>Rejected</th>
                                <th>Conv. Rate</th>
                                <th>Today Breakdown</th>
                              </tr>
                            </thead>
                            <tbody>
                              {[...tcStats].sort((a, b) => b.allConverted - a.allConverted).map(tc => {
                                const convRate = tc.allTotal ? Math.round((tc.allConverted / tc.allTotal) * 100) : 0;
                                const isMe = tc.id === currentUser?.id;
                                return (
                                  <tr key={tc.id}
                                    style={{ background: isMe ? 'rgba(61,127,255,0.05)' : undefined, cursor: 'pointer' }}
                                    onClick={() => setDetailModal({ type: 'meetings-list', title: `${tc.name} — All Meetings`, data: { meetings: meetings.filter(m => m.tcId === tc.id), subtitle: `${tc.allTotal} total · ${tc.allConverted} converted` } })}>
                                    <td className="primary" style={{ color: isMe ? '#3d7fff' : 'var(--text)' }}>
                                      {tc.name}
                                      {isMe && <span style={{ fontSize: '9px', background: 'rgba(61,127,255,0.15)', color: '#3d7fff', padding: '1px 6px', borderRadius: '4px', marginLeft: '6px', fontFamily: "'JetBrains Mono', monospace" }}>YOU</span>}
                                    </td>
                                    <td style={{ color: '#3d7fff', fontWeight: 700 }}>{tc.allTotal}</td>
                                    <td style={{ color: COLORS.converted, fontWeight: 700 }}>{tc.allConverted}</td>
                                    <td style={{ color: COLORS.pending, fontWeight: 600 }}>{tc.allPending}</td>
                                    <td style={{ color: COLORS.rejected, fontWeight: 600 }}>{tc.allRejected}</td>
                                    <td>
                                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        <div style={{ width: '64px', height: '6px', background: 'var(--surface2)', borderRadius: '3px', overflow: 'hidden' }}>
                                          <div style={{ height: '100%', width: `${convRate}%`, borderRadius: '3px', background: convRate >= 60 ? COLORS.converted : convRate >= 30 ? COLORS.pending : COLORS.rejected, transition: 'width 0.5s' }} />
                                        </div>
                                        <span style={{ fontSize: '11px', fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", color: convRate >= 60 ? COLORS.converted : convRate >= 30 ? COLORS.pending : COLORS.rejected }}>{convRate}%</span>
                                      </div>
                                    </td>
                                    <td>
                                      {tc.todayTotal === 0
                                        ? <span style={{ fontSize: '10px', color: 'var(--text3)', fontFamily: "'JetBrains Mono', monospace" }}>no meetings today</span>
                                        : <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                                          {[
                                            { v: tc.scheduled, c: COLORS.scheduled, l: 'Sched' },
                                            { v: tc.converted, c: COLORS.converted, l: 'Done' },
                                            { v: tc.pending, c: COLORS.pending, l: 'Pend' },
                                            { v: tc.rescheduled, c: COLORS.rescheduled, l: 'Resched' },
                                            { v: tc.rejected, c: COLORS.rejected, l: 'Rej' },
                                          ].filter(x => x.v > 0).map(x => (
                                            <span key={x.l} style={{ fontSize: '10px', fontWeight: 700, padding: '2px 7px', borderRadius: '5px', background: x.c + '20', color: x.c, fontFamily: "'JetBrains Mono', monospace", border: `1px solid ${x.c}30` }}>
                                              {x.l}: {x.v}
                                            </span>
                                          ))
                                          }</div>
                      ...
                      {meetingRequests.filter(mr => mr.tcId === currentUser?.id).map(req => {
                        ...
                        return (
                          <tr key={req.id} style={{ cursor: 'pointer' }}
                            onClick={() => setDetailModal({ type: 'request-detail', title: lead?.clientName || 'Lead Details', data: { req, lead, bo, subtitle: `Request by ${bo?.name} · ${req.status}` } })}>
                ...
                {approvedPending.map(req => {
                  ...
                  return (
                    <div key={req.id} className="glass-card">
                      <div className="card-header">
                        <div>
                          <div className="card-title">{lead.clientName}</div>
                          <div className="card-sub">// BO: {bo?.name}</div>
                        </div>
                  ...
                  <table className="data-table">
                    ...
                    <tbody>
                      {meetingsByDate.slice().reverse().map(([date, count]) => {
                        ...
                        return (
                          <tr key={date} style={{ cursor: 'pointer' }}
                            onClick={() => setDetailModal({ type: 'date-detail', title: `Meetings on ${date}`, data: { meetings: dm, subtitle: `${count} meetings · ${conv} converted · ${convRate}% rate` } })}>
