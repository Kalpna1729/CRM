...
export default function BODashboard() {
  ...
  // Lock stable order when new leads arrive (not on every status update)
  useEffect(() => {
    const todayIds = todayLeads.map(l => l.id);
    setStableLeadOrder(prev => {
      const newIds = todayIds.filter(id => !prev.includes(id));
      return [...prev, ...newIds];
    });
  }, [todayLeads.length]);
  ...
  // Filtered today's leads — stable order maintained
  const filteredTodayLeads = useMemo(() => {
    let filtered = todayLeads;
    if (statusFilter !== 'all') filtered = filtered.filter(l => l.leadStatus === statusFilter);
    if (numberStatusFilter !== 'all') filtered = filtered.filter(l => l.numberStatus === numberStatusFilter);
    return [...filtered].sort((a, b) =>
      stableLeadOrder.indexOf(a.id) - stableLeadOrder.indexOf(b.id)
    );
  ...
  // Status drill-down leads — stable order maintained
  const drillDownLeads = useMemo(() => {
    if (!statusDrillDown) return [];
    const filtered = todayLeads.filter(l => l.numberStatus === statusDrillDown);
    return [...filtered].sort((a, b) =>
      stableLeadOrder.indexOf(a.id) - stableLeadOrder.indexOf(b.id)
    );
  }, [todayLeads, statusDrillDown, stableLeadOrder]);
        ...
        <Table>
          ...
          <TableBody>
            {leadsList.map(lead => {
              const remarks = getLeadRemarks(lead.id);
              return (
                <TableRow key={lead.id} className={lead.meetingRejected ? "bg-red-100 dark:bg-red-950/20" : ""}>
                  <TableCell className="font-medium">{lead.clientName}</TableCell>
                  <TableCell>{lead.phoneNumber}</TableCell>
                  <TableCell>₹{lead.loanRequirement}</TableCell>
                  <TableCell>
