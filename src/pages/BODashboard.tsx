            ...
            {leadsList.map(lead => {
              ...
              return (
                <TableRow key={lead.id}>
                  ...
                  <TableCell>
                    <Select value={lead.leadStatus || undefined} onValueChange={v => updateLeadStatus(lead.id, v as LeadStatus)}>
                      <SelectTrigger className="w-36 h-8 text-xs"><SelectValue placeholder="Set status" /></SelectTrigger>
                      <SelectContent>{leadStatuses.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
                    </Select>
                  </TableCell>
                  <TableCell>
                    <Select value={lead.leadType || undefined} onValueChange={v => updateLeadType(lead.id, v as LeadType)}>
                    ...
                    {showRemarks && (
                      <div className="space-y-1">
                        ...
                        <div className="flex gap-1">
                          ...
                          />
                          <Button size="sm" variant="ghost" className="h-6 px-1" onClick={() => handleAddRemark(lead.id)}>
                  ...
                  <TableCell>
                    {lead.leadStatus === 'Interested' && !lead.meetingRequested && (
                      <Button size="sm" variant="outline" onClick={() => requestMeeting(lead.id)}>
                        <Send className="w-3 h-3 mr-1" />Request Meeting
                      </Button>
                    )}
