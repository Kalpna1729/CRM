...
const Icons = {
  ...
  remarks: <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z" /></svg>,
  ...
  send: <svg width="11" height="11" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><line x1="22" y1="2" x2="11" y2="13" /><polygon points="22 2 15 22 11 13 2 9 22 2" /></svg>,
...
// ─── Role colour map for remark tags ─────────────────────────────────────────
const ROLE_BADGE_STYLE: Record<string, { bg: string; color: string }> = {
  BDM: { bg: 'rgba(61,127,255,0.13)',  color: '#3d7fff' },
  BDO: { bg: 'rgba(167,139,250,0.13)', color: '#a78bfa' },
  BO:  { bg: 'rgba(0,212,170,0.13)',   color: '#00d4aa' },
  TC:  { bg: 'rgba(245,158,11,0.13)',  color: '#f59e0b' },
  FM:  { bg: 'rgba(255,107,53,0.13)',  color: '#ff6b35' },
  RM:  { bg: 'rgba(236,72,153,0.13)',  color: '#ec4899' },
  FO:  { bg: 'rgba(99,102,241,0.13)',  color: '#6366f1' },
};
...
const statusBadge = (status: string) => {
  const map: Record<string, string> = {
    ...
    'Scheduled': 'badge-scheduled', 'Reject': 'badge-notdone',
    'Reschedule Requested': 'badge-reschedule', 'Reschedule Sent': 'badge-reschedule',
    'Connected': 'badge-connected', 'Not Connected': 'badge-notconn',
    'Follow-up': 'badge-followup', 'Walk-in': 'badge-followup',
  };
  return <span className={`badge ${map[status] || 'badge-pending'}`}>{status}</span>;
};

// ─── Remark Row Component ─────────────────────────────────────────────────────
function RemarkRow({ r, users }: { r: any; users: any[] }) {
  // Try to find user by name (createdBy is name string in meeting_remarks)
  const userMatch = users.find(u =>
    u.name === r.createdBy || u.id === r.createdBy
  );
  const role = userMatch?.role || '';
  const roleStyle = ROLE_BADGE_STYLE[role] || { bg: 'rgba(148,163,184,0.13)', color: '#94a3b8' };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {role || r.createdBy?.[0]?.toUpperCase() || '?'}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        {/* Header line */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px', flexWrap: 'wrap' }}>
          <span style={{ fontSize: '12px', fontWeight: 700, color: 'var(--text)' }}>{r.createdBy}</span>
          {role && (
            <span style={{
          ...
          <span style={{ fontSize: '10px', color: 'var(--text3)', fontFamily: 'monospace', marginLeft: 'auto' }}>
            {dateStr} · {timeStr}
          </span>
        </div>
        {/* Remark text */}
        <p style={{ fontSize: '12px', color: 'var(--text2)', lineHeight: 1.5, margin: 0 }}>{r.remark}</p>
      </div>
    </div>
  );
}
...
export default function BDMDashboard() {
  ...
  // ── Expanded remarks panel per meeting ──
  const [expandedRemarks, setExpandedRemarks]   = useState<Record<string, boolean>>({});
  // ── Draft remark text per meeting ──
  const [remarkDrafts, setRemarkDrafts]         = useState<Record<string, string>>({});
  // ── Submitting state per meeting ──
  ...
  const pendingMeetings  = useMemo(() => myMeetings.filter(m => m.status === 'Pending'), [myMeetings]);
  ...
  // ── Remark handlers ──────────────────────────────────────────────────────
  const toggleRemarks = (meetingId: string) => {
    setExpandedRemarks(prev => ({ ...prev, [meetingId]: !prev[meetingId] }));
  };

  const handleAddRemark = async (meetingId: string) => {
    const text = (remarkDrafts[meetingId] || '').trim();
    if (!text) { toast.error('Remark cannot be empty'); return; }
    setSubmitting(prev => ({ ...prev, [meetingId]: true }));
    // Pass currentUser name + role so it's identifiable in the remark list
    const byLabel = currentUser?.name
      ? `${currentUser.name}`
      : 'BDM';
    await addMeetingRemark(meetingId, text, byLabel);
    setRemarkDrafts(prev => ({ ...prev, [meetingId]: '' }));
  ...
  const switchTab = (tab: string) => { setActiveTab(tab); setDetailView(null); };

  // ─── Meeting table — now with inline remark panel ─────────────────────────
    ...
    <table className="data-table" style={{ tableLayout: 'fixed', width: '100%' }}>
      ...
      <tbody>
        {meetingsList.map(m => {
          ...
          const isExpanded  = !!expandedRemarks[m.id];
          const draft       = remarkDrafts[m.id] || '';
          const isSending   = !!submitting[m.id];

          return (
            <React.Fragment key={m.id}>
              {/* ── Main row ── */}
              <tr style={{ background: isExpanded ? 'var(--surface2)' : undefined }}>
                <td style={{ fontFamily: 'monospace', fontSize: '11px', color: 'var(--accent)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.date}</td>
                <td style={{ fontFamily: 'monospace', fontSize: '11px', fontWeight: 700, color: 'var(--text)' }}>{m.timeSlot}</td>
                <td className="primary" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{lead?.clientName}</td>
                <td style={{ fontFamily: 'monospace', fontSize: '10px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{lead?.phoneNumber}</td>
                ...
                <td style={{ textAlign: 'center', padding: '8px 4px' }}>
                  ...
                  >
                    {Icons.remarks}
                    {mRemarks.length > 0 ? ` ${mRemarks.length}` : ' Add'}
              ...
              {isExpanded && (
                <tr>
                  <td colSpan={12} style={{ padding: 0, background: 'var(--bg3)', borderBottom: '2px solid var(--accent)22' }}>
                    <div style={{ padding: '16px 20px' }}>

                      {/* Header */}
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                        <span style={{ fontSize: '11px', fontWeight: 700, color: 'var(--text)', letterSpacing: '1px', textTransform: 'uppercase', fontFamily: 'monospace' }}>
                          Remarks — {lead?.clientName}
                        </span>
                        <span style={{
                          fontSize: '9px', fontWeight: 700, padding: '2px 8px', borderRadius: '20px',
                          background: 'rgba(61,127,255,0.1)', color: 'var(--accent)',
                          border: '1px solid rgba(61,127,255,0.2)', fontFamily: 'monospace',
                        }}>
                          {mRemarks.length} remark{mRemarks.length !== 1 ? 's' : ''}
                        </span>
                        <span style={{ marginLeft: 'auto', fontSize: '10px', color: 'var(--text3)', fontFamily: 'monospace' }}>
                          Each remark is tagged by role — BDM / BDO / BO
                        </span>
                      </div>
                      ...
                      {mRemarks.length > 0 ? (
                        <div style={{
                          ...
                          {mRemarks.map(r => (
                            <RemarkRow key={r.id} r={r} users={users} />
                          ))}
                      ...
                      <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-end' }}>
                        {/* Who is adding - visual indicator */}
                        <div style={{
                          ...
                          <div style={{
                            ...
                            background: ROLE_BADGE_STYLE['BDM'].bg,
                            color: ROLE_BADGE_STYLE['BDM'].color,
                            border: `1px solid ${ROLE_BADGE_STYLE['BDM'].color}33`,
                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                            fontSize: '9px', fontWeight: 700, letterSpacing: '0.5px',
                          }}>BDM</div>
                        ...
                        <textarea
                          placeholder="Type your remark here... (Ctrl+Enter to submit)"
                          value={draft}
                          onChange={e => setRemarkDrafts(prev => ({ ...prev, [m.id]: e.target.value }))}
                          onKeyDown={e => { if (e.key === 'Enter' && e.ctrlKey) handleAddRemark(m.id); }}
                          style={{
                            flex: 1, minHeight: '70px', resize: 'vertical',
                        ...
                        <button
                          ...
                          style={{
                            ...
                            background: draft.trim() ? 'rgba(61,127,255,0.1)' : 'var(--surface2)',
                            color: draft.trim() ? 'var(--accent)' : 'var(--text3)',
                            border: draft.trim() ? '1px solid rgba(61,127,255,0.3)' : '1px solid var(--border)',
                            cursor: draft.trim() ? 'pointer' : 'not-allowed',
                            fontSize: '11px', fontWeight: 600, fontFamily: 'monospace',
                            opacity: isSending ? 0.6 : 1, transition: 'all 0.15s',
                            whiteSpace: 'nowrap',
                          }}
                        >
                          {isSending ? '...' : Icons.send}
                          {isSending ? 'Saving' : 'Save Remark'}
                        </button>
                      </div>

                      {/* Role legend */}
                      <div style={{ display: 'flex', gap: '10px', marginTop: '10px', flexWrap: 'wrap' }}>
                        <span style={{ fontSize: '9px', color: 'var(--text3)', fontFamily: 'monospace' }}>Colour key:</span>
                        {Object.entries(ROLE_BADGE_STYLE).map(([role, s]) => (
                          <span key={role} style={{
                            fontSize: '9px', fontWeight: 700, padding: '1px 7px',
        ...
        {meetingsList.length === 0 && (
          <tr><td colSpan={12} className="empty-row">no meetings found</td></tr>
        )}
      </tbody>
    </table>
  );
  ...
  return (
    <>
      <style>{`
        ...
        .bdm-root.dark {
          --bg:#07080f;--bg2:#0d0f1a;--bg3:#12152a;--surface:#161929;--surface2:#1c2038;
          --border:rgba(255,255,255,0.06);--border2:rgba(255,255,255,0.1);
          --accent:#3d7fff;--success:#00d4aa;--warning:#f59e0b;--danger:#ff4757;--purple:#a78bfa;--orange:#ff6b35;
          --text:#e8eaf6;--text2:#8892b0;--text3:#4a5568;
        }
        .bdm-root.light {
          --bg:#f4f5fa;--bg2:#ffffff;--bg3:#eef0f7;--surface:#ffffff;--surface2:#eef0f7;
          --border:rgba(0,0,0,0.07);--border2:rgba(0,0,0,0.12);
          --accent:#2563eb;--success:#059669;--warning:#d97706;--danger:#dc2626;--purple:#7c3aed;--orange:#ea580c;
          --text:#0f172a;--text2:#475569;--text3:#94a3b8;
        }

        .bdm-root { font-family:'Syne',sans-serif;background:var(--bg);color:var(--text);min-height:100vh;position:relative;transition:background 0.25s,color 0.25s; }
        .bdm-root.dark::before { content:'';position:fixed;top:-50%;left:-50%;width:200%;height:200%;background:radial-gradient(ellipse 600px 400px at 20% 30%,rgba(61,127,255,0.06),transparent),radial-gradient(ellipse 500px 500px at 80% 70%,rgba(0,212,170,0.05),transparent);pointer-events:none;z-index:0; }
        .bdm-root.dark::after { content:'';position:fixed;inset:0;background-image:linear-gradient(rgba(255,255,255,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,0.03) 1px,transparent 1px);background-size:60px 60px;pointer-events:none;z-index:0;opacity:0.4; }

        .cc-layout { display:flex;min-height:100vh;position:relative;z-index:1; }

        .cc-sidebar { width:240px;flex-shrink:0;background:var(--bg2);border-right:1px solid var(--border);display:flex;flex-direction:column;position:sticky;top:0;height:100vh;overflow:hidden;transition:background 0.25s; }
        ...
        .cc-nav-badge { margin-left:auto;font-size:9px;font-weight:700;background:rgba(245,158,11,0.15);color:var(--warning);border:1px solid rgba(245,158,11,0.28);padding:1px 7px;border-radius:20px;font-family:'JetBrains Mono',monospace;line-height:1.6; }
        ...
        .cc-kpi.orange { border-color:rgba(255,107,53,0.13); } .cc-kpi.orange:hover { border-color:rgba(255,107,53,0.33);box-shadow:0 0 40px rgba(255,107,53,0.13); }
        .cc-kpi.yellow { border-color:rgba(245,158,11,0.13); } .cc-kpi.yellow:hover { border-color:rgba(245,158,11,0.33);box-shadow:0 0 40px rgba(245,158,11,0.13); }
        .cc-kpi.red { border-color:rgba(255,71,87,0.13); } .cc-kpi.red:hover { border-color:rgba(255,71,87,0.33);box-shadow:0 0 40px rgba(255,71,87,0.13); }
        .cc-kpi::before { content:'';position:absolute;top:-40px;right:-40px;width:120px;height:120px;border-radius:50%;opacity:0.06; }
        .cc-kpi.orange::before { background:var(--orange); } .cc-kpi.yellow::before { background:var(--warning); } .cc-kpi.red::before { background:var(--danger); }
        ...
        .cc-kpi.orange .cc-kpi-value { color:var(--orange); } .cc-kpi.yellow .cc-kpi-value { color:var(--warning); } .cc-kpi.red .cc-kpi-value { color:var(--danger); }
        ...
        .modal-box { background:var(--bg2);border:1px solid var(--border2);border-radius:20px;width:460px;max-width:95vw;padding:28px;position:relative;box-shadow:0 24px 60px rgba(0,0,0,0.4); }
        ...
        .remark-toggle-btn:hover { opacity:0.85; }
        ...
        <div className="cc-layout">
          ...
          <aside className="cc-sidebar">
            ...
            <div className="cc-nav-section">
              ...
              <div className={`cc-nav-item ${activeTab === 'dashboard' ? 'active' : ''}`} onClick={() => switchTab('dashboard')}>
                <div className="cc-nav-icon">{Icons.dashboard}</div>Dashboard
              </div>
              <div className={`cc-nav-item ${activeTab === 'pending-meetings' ? 'active' : ''}`} onClick={() => switchTab('pending-meetings')}>
                <div className="cc-nav-icon">{Icons.pending}</div>
                Pending Meetings
                {pendingMeetings.length > 0 && <span className="cc-nav-badge">{pendingMeetings.length}</span>}
              </div>
              <div className={`cc-nav-item ${activeTab === 'meetings' ? 'active' : ''}`} onClick={() => switchTab('meetings')}>
                <div className="cc-nav-icon">{Icons.meetings}</div>All Meetings
              </div>
            </div>
            <div className="cc-sidebar-footer">
              <div className="cc-footer-info">
                <span className="cc-status-dot" />Active · {todayStr}<br />
                <span style={{ color: 'var(--text3)', display: 'block' }}>{rescheduled} reschedule · {pendingMeetings.length} pending</span>
                <span style={{ color: 'var(--text3)', display: 'block' }}>{totalMeetings} total meetings</span>
              </div>
              ...
              <button className="cc-logout-btn" onClick={logout}>{Icons.logout}&nbsp;Sign Out</button>
            </div>
          </aside>
          ...
          <main className="cc-main">
              ...
              <div className="fade-in">
                ...
                {rescheduled > 0 && !detailView && (
                  <div className="cc-alert-strip">
                    ...
                    <div className="cc-alert-text"><strong>{rescheduled} reschedule request{rescheduled > 1 ? 's' : ''}</strong> waiting — TC needs to be notified.</div>
                  </div>
                )}

                {detailView ? (
                  <>
                    <button className="back-btn" onClick={() => setDetailView(null)}>{Icons.back} back to dashboard</button>
                    <div className="glass-card">
                      <div className="card-header">
                        <div><div className="card-title">{detailTitle[detailView]}</div><div className="card-sub">// {getDetailMeetings().length} meetings</div></div>
                      </div>
                      <div className="table-scroll">{renderMeetingTable(getDetailMeetings(), true)}</div>
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <div className="glass-card">
                      <div className="card-header">
                        <div><div className="card-title">Today's Meetings</div><div className="card-sub">// {todayStr} · {todayMeetings.length} scheduled</div></div>
                      </div>
                      <div className="table-scroll">{renderMeetingTable(todayMeetings, true)}</div>
                    </div>

                    {upcomingMeetings.length > 0 && (
                      <div className="glass-card">
                        <div className="card-header">
                          <div><div className="card-title">Upcoming Meetings</div><div className="card-sub">// {upcomingMeetings.length} scheduled ahead</div></div>
                        </div>
                        <div className="table-scroll">{renderMeetingTable(upcomingMeetings, true)}</div>
                      </div>
                    )}
                  </>
                )}
              ...
              <div className="fade-in">
                <div className="cc-topbar">
                  <div>
                    ...
                    <div className="cc-page-sub">// meetings awaiting BDO action · {pendingMeetings.length} total</div>
                  </div>
                  <div className="cc-topbar-right"><div className="cc-clock">{clock}</div></div>
                </div>
                <div className="cc-date-filter">
                  <span className="date-label">FROM</span>
                  <input type="date" className="cc-date-input" value={fromDate} onChange={e => setFromDate(e.target.value)} />
                ...
                {pendingMeetings.length > 0 && (
                  <div className="cc-pending-strip">
                    <span style={{ fontSize: '16px' }}>⏳</span>
                    <div className="cc-pending-strip-text"><strong>{pendingMeetings.length} pending meeting{pendingMeetings.length > 1 ? 's' : ''}</strong> — assigned to BDO, awaiting follow-up.</div>
                  </div>
                )}
                <div className="glass-card">
                  <div className="card-header">
                    <div><div className="card-title">Pending Meetings</div><div className="card-sub">// status = Pending · {pendingMeetings.length} meetings</div></div>
                  </div>
                  <div className="table-scroll">{renderMeetingTable(pendingMeetings, true)}</div>
                </div>
              </div>
            )}
            ...
            {activeTab === 'meetings' && (
              <div className="fade-in">
                ...
                <div className="glass-card">
                  <div className="card-header">
                    <div><div className="card-title">Meeting History</div><div className="card-sub">// {myMeetings.length} meetings in selected range</div></div>
                  </div>
                  <div className="table-scroll">{renderMeetingTable(myMeetings, true)}</div>
                </div>
              </div>
            )}
          ...
          <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) { setPendingAssignMeeting(null); setSelectedBdoId(''); } }}>
            <div className="modal-box">
              ...
              <div className="modal-footer">
                <button className="action-btn btn-cancel" onClick={() => { setPendingAssignMeeting(null); setSelectedBdoId(''); }}>{Icons.x} Cancel</button>
                <button
                  className="action-btn btn-approve"
                  style={{ opacity: selectedBdoId ? 1 : 0.4, cursor: selectedBdoId ? 'pointer' : 'not-allowed' }}
                  onClick={handleConfirmPendingAssignment}
                >{Icons.check} Confirm & Assign BDO</button>
              </div>
            </div>
          </div>
        );
      ...
      {viewFormLeadId && (() => {
        ...
        const labelStyle: React.CSSProperties = { fontSize: '9px', fontWeight: 600, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '1px', fontFamily: 'monospace', marginBottom: '4px', display: 'block' };
        const valStyle = { fontSize: '13px', color: 'var(--text)', fontWeight: 500 };
        const sectionStyle: React.CSSProperties = { fontSize: '11px', fontWeight: 700, color: 'var(--accent)', fontFamily: 'monospace', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '12px', marginTop: '8px', paddingBottom: '6px', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'}` };
        ...
        const Field = ({ label, k }: { label: string; k: string }) => (<div style={fieldStyle}><label style={labelStyle}>{label}</label><div style={valStyle}>{val(k)}</div></div>);

        return (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1002, padding: '20px' }}>
            <div id="bdm-form-print-area" style={{ background: isDark ? '#0d0f1a' : '#fff', border: `1px solid ${isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'}`, borderRadius: '16px', width: '100%', maxWidth: '820px', maxHeight: '90vh', overflow: 'auto' }}>
              <div style={{ padding: '18px 24px', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, background: isDark ? '#0d0f1a' : '#fff', zIndex: 1 }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: '16px' }}>Client Detail Form</div>
                  <div style={{ fontSize: '11px', color: 'var(--text3)', fontFamily: 'monospace' }}>{lead.clientName} · ₹{lead.loanRequirement}</div>
                </div>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button className="action-btn" style={{ background: 'rgba(255,71,87,0.1)', color: 'var(--danger)', borderColor: 'rgba(255,71,87,0.2)' }} onClick={() => setViewFormLeadId(null)}>✕ Close</button>
                </div>
              </div>
              <div style={{ padding: '24px' }}>
                <div style={sectionStyle}>Basic Information</div>
                <div style={grid2}><Field label="Client Name" k="clientName" /><Field label="Contact Number" k="contactNumber" /><Field label="Email" k="email" /><Field label="State" k="state" /></div>
                <div style={{ ...fieldStyle, marginBottom: '14px' }}><label style={labelStyle}>Address</label><div style={valStyle}>{val('address')}</div></div>
                <div style={sectionStyle}>Business Information</div>
                <div style={grid2}><Field label="Entity Name" k="entityName" /><Field label="Entity Type" k="entityType" /><Field label="Nature of Business" k="natureOfBusiness" /><Field label="Business Place" k="businessPlace" /><Field label="Business Vintage" k="businessVintage" /><Field label="DSA Name" k="dsaName" /></div>
                <div style={{ ...fieldStyle, marginBottom: '14px' }}><label style={labelStyle}>Business Description</label><div style={valStyle}>{val('businessDescription')}</div></div>
                <div style={sectionStyle}>Financial Information</div>
                <div style={grid3}><Field label="Last Year Turnover" k="lastYearTurnover" /><Field label="Last Year Net Profit" k="lastYearNetProfit" /><Field label="Liability Amount" k="liabilityAmount" /><Field label="Bank Name" k="bankName" /><Field label="Loan Amount Status" k="loanAmountStatus" /></div>
                <div style={sectionStyle}>Loan Requirement</div>
                <div style={grid2}><Field label="Requirement Type" k="requirementType" /><Field label="Required Amount" k="requiredAmount" /><Field label="Collateral Type" k="collateralType" /><Field label="Collateral Value" k="collateralValue" /></div>
                <div style={{ ...fieldStyle, marginBottom: '14px' }}><label style={labelStyle}>Collateral Description</label><div style={valStyle}>{val('collateralDescription')}</div></div>
                <div style={fieldStyle}><label style={labelStyle}>Project Description</label><div style={valStyle}>{val('projectDescription')}</div></div>
              </div>
            </div>
          </div>
        );
