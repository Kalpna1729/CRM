...
// 'Meeting Done' hataya, 'Reschedule' add kiya
...
export default function BDMDashboard() {
  ...
  // Stats: 'Reschedule Requested' ko reschedule count mein
  ...
  const handleStatusChange = (meeting: Meeting, status: BDMStatus) => {
    if (status === 'Pending') {
      ...
      // Meeting ko 'Reschedule Requested' mark karo — TC ke paas wapas jaayegi
    ...
    <Table>
      ...
      <TableBody>
        {meetingsList.map(m => {
          ...
          // Agar already reschedule requested hai toh dropdown mat dikhao
                ...
                <TableCell>
                  {isRescheduleRequested ? (
                    <Badge variant="outline" className="gap-1 text-orange-600 border-orange-400">
                      <RefreshCw className="w-3 h-3" />Reschedule Sent
                    </Badge>
                  ) : (
                    <Select
                      value={m.status === 'Scheduled' ? undefined : m.status}
                      onValueChange={v => handleStatusChange(m, v as BDMStatus)}
                    >
                      <SelectTrigger className="w-36 h-8 text-xs"><SelectValue placeholder="Update Status" /></SelectTrigger>
                      <SelectContent>
                        {step1Statuses.map(s => (
                          <SelectItem key={s} value={s}>
                            {s === 'Reschedule' ? '🔄 Reschedule' : s}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
      ...
      {activeTab === 'dashboard' && (
        <div className="space-y-6">
          ...
          {detailView ? (
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {/* Meeting Done ki jagah Reschedule Requested */}
                <StatCard label="Reschedule Requested" value={rescheduled} variant="primary" onClick={() => setDetailView('rescheduled')} />
                <StatCard label="Pending" value={pending} variant="info" onClick={() => setDetailView('pending')} />
                <StatCard label="Rejected" value={rejected} variant="destructive" onClick={() => setDetailView('rejected')} />
              </div>
